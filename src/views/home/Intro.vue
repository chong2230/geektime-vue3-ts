...
import { getSaleType, getSaleLabel } from '@/common/util'
import api from '@/common/api'
import intro from '@/mock/home/intro';
...
export default {
    ...
    setup() {
        ...
        async function getDetail() {
            ...
            const res = await api.getColumnIntro({ cid: id, with_groupbuy: true });
            // 无效的专栏ID
            if (res.code == 0) {
        ...
        async function getChapters() {
            const { id } = route.params;
            const res = await api.getColumnChapters({ cid: id });
            if (res.code == 0 && res.data.length > 0) {
                state.chapters = res.data;
            } else {
                let chapters: any = intro['/serv/v1/chapters'].data
        ...
        async function getArticles() {
            const { id } = route.params;
            const res = await api.getColumnArticles({ cid: id, order: 'earliest', prev: 0, sample: false, size: 500 });
            // 无效的专栏ID
            if (res.code == 0) {
                state.articles = res.data.list;
            } else {
                let articles: any = [];
                articles = intro['/serv/v1/column/articles'].data.list;
                state.articles = articles;//intro['/serv/v1/column/articles'].data.list;
            }
            let arr: any = new Array(state.chapters.length).fill(0);
            state.chapters.forEach((val: any, index: number)=>{
                for (let article of state.articles) {
                    let obj: any = article;
                    let cid: number | string = obj.chapter_id;
                    let vid: number | string = val.id;
                    if (cid == vid) arr[index]++;
                }
            });
        ...
        async function getRecommend() {
            let { id } = route.params;
            const res = await api.getColumnRecommend({product_id: parseInt(id as string)});
            if (res.code == 0) {
        ...
        const getArticleLink = (id: number | string) => {
            return 'https://time.geekbang.org/column/article/' + id;
        }

        const goStore = (type: number) => {
        ...
        const goColumn = (id: number | string) => {
